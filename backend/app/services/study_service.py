...
from ..models import Study, Participant, QSortEntry, ParticipantStatus, Statement, StudyTranslation
from ..schemas import SubmissionInput
from ..utils.crypto import hash_ip

class StudyService:
    @staticmethod
    async def get_study_by_slug(db: AsyncSession, slug: str) -> Optional[Study]:
    ...
    @staticmethod
    def resolve_translation(study: Study, requested_lang: str) -> (str, Optional[StudyTranslation]):
        """
        Logic: Requested Lang -> Default (Study) -> English -> First Available
        """
        # 1. Requested
        translation = next((t for t in study.translations if t.language_code == requested_lang), None)
    ...
    def validate_distribution(study: Study, qsort: List[Any]):
        ...
        if len(qsort) != stmt_count:
             raise HTTPException(status_code=400, detail=f"Submission incomplete. Expected {stmt_count} cards, got {len(qsort)}.")
    ...
    @staticmethod
    async def process_submission(db: AsyncSession, data: SubmissionInput, client_ip: str):
        # 1. IP Hashing
        ...
        participant_stmt = select(Participant).where(Participant.session_token == data.session_token)
        ...
        if not participant:
            ...
            await db.execute(delete(QSortEntry).where(QSortEntry.participant_id == participant.id))
            await db.flush()
