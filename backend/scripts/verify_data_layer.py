...
"""Verification script for the data layer integration."""
...
async def test_workflow():
    """Run via database integration test workflow."""
    async with SessionLocal() as session:
        # 1. Create Owner
        owner = User(
            email="admin@example.com", hashed_password="hashed_secret", is_active=True
        )
        session.add(owner)
        ...
        try:
            ...
            print(
                "4. Constraint Verified: Cannot duplicate StudyTranslation for same language."
            )
        ...
        st_t1 = StatementTranslation(
            statement_id=stmt1.id, language_code="en", text="I like coding."
        )
        st_t2 = StatementTranslation(
            statement_id=stmt2.id, language_code="en", text="I like testing."
        )
        ...
        participant = Participant(
            study_id=study.id, language_used="en", presort_answers={"age": 25}
        )
        ...
        q1 = QSortEntry(
            ...
            grid_score=2,
            card_comment="Love it",
        )
        q2 = QSortEntry(
            participant_id=participant.id, statement_id=stmt2.id, grid_score=-1
        )
        ...
        try:
            q_fail = QSortEntry(
                participant_id=participant.id, statement_id=stmt1.id, grid_score=0
            )
            session.add(q_fail)
            await session.commit()
        except IntegrityError:
            print(
                "8. Constraint Verified: Participant cannot sort same statement twice."
            )
            await session.rollback()
            await session.refresh(study)
