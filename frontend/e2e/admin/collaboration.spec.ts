...
test.describe('Collaboration Integration', () => {
    ...
    test('Admin can invite existing user via full invitation flow', async ({ page, testDb, authToken }) => {
        // 1. Seed second user (User B)
        const seedResponse = await fetch(`${apiUrl}/api/test/seed`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                user: { email: collabEmail, password: collabPassword, is_superuser: false },
        ...
        const loginResponse = await fetch(`${apiUrl}/api/token`, {
            ...
            body: new URLSearchParams({ username: collabEmail, password: collabPassword })
        });
