...
test.describe('Interface Customization Testing', () => {
    test.describe('UI Labels', () => {
        ...
        test('Admin: Can customize button labels', async ({ page }) => {
            ...
            await expect(page.locator('input[name="label_agree"]')).toHaveValue('I Agree');
        });
        ...
        test('Validation: Empty labels not allowed', async ({ page }) => {
            ...
            const value = await page.locator('input[name="label_agree"]').inputValue();
            expect(value).toBeTruthy(); // Should not be empty
        });

        test('Edge Case: Long custom labels', async ({ page, testDb, authToken }) => {
            const longLabel = 'This is a very long custom label that might cause layout issues in the UI';
    ...
    test.describe('Process Steps Customization', () => {
        ...
        test('Admin: Can customize process step names', async ({ page }) => {
            ...
            // Verify
            await expect(page.locator('input[name="step_finesort"]')).toHaveValue('Final Arrangement');
        });
        ...
        test('Participant: Custom step names in navigation', async ({ page, testDb, authToken }) => {
            await testDb.updateStudy(authToken, studySlug, {
                translations: [{
                    language_code: 'en',
                    title: 'Test Study',
            ...
            const hasCustomLabel = await page.locator('text=Custom Rough Sort, text=Custom Fine Sort').count();
    ...
    test.describe('Help Text Customization', () => {
        ...
        test('Admin: Can add help text for steps', async ({ page }) => {
            ...
            // Add help text
            await page.fill('textarea[name="help_roughsort"]', 'Sort statements into three piles...');
            await page.fill('textarea[name="help_finesort"]', 'Arrange statements in the grid...');

            // Verify
            await expect(page.locator('textarea[name="help_finesort"]'))
                .toHaveValue('Arrange statements in the grid...');
        });
    ...
    test.describe('Statement Code Display', () => {
        ...
        test('Participant: Statement codes shown when enabled', async ({ page, testDb, authToken }) => {
            await testDb.updateStudy(authToken, studySlug, {
                show_statement_codes: true,
                state: 'active',
            });
        ...
        test('Participant: Statement codes hidden when disabled', async ({ page, testDb, authToken }) => {
            await testDb.updateStudy(authToken, studySlug, {
                show_statement_codes: false,
                state: 'active',
            });
