...
test.describe('Post-Sort Configuration Testing', () => {
    test.describe('Email Collection', () => {
        ...
        test('Participant: Email field appears when enabled', async ({ page, testDb, authToken }) => {
        ...
        test('Validation: Email validation works', async ({ page, testDb, authToken }) => {
            ...
            await expect(page.locator('text=valid email', { hasText: /valid.*email/i })).toBeVisible();
    ...
    test.describe('Consent Options', () => {
        ...
        test.beforeEach(async ({ testDb, authToken }) => {            const study = await testDb.createStudy(authToken, testDataBuilders.study({
        ...
        test('Participant: Consent checkboxes appear when enabled', async ({ page, testDb, authToken }) => {
            await testDb.updateStudy(authToken, studySlug, {
                postsort_config: {
                    interview_consent_enabled: true,
                    newsletter_consent_enabled: true,
            ...
            await expect(page.locator('input[type="checkbox"][name="interview_consent"]')).toBeVisible();
            await expect(page.locator('input[type="checkbox"][name="newsletter_consent"]')).toBeVisible();
        });
    });

    test.describe('Custom Questions', () => {
        const QUESTION_TYPES: PresortFieldType[] = ['text', 'textarea', 'select', 'radio', 'checkbox'];
        ...
        for (const questionType of QUESTION_TYPES) {
            test.describe(`Question Type: ${questionType}`, () => {
                ...
                test(`Admin: Can add ${questionType} question`, async ({ page }) => {
                    ...
                    await page.fill('input[name="questionLabel"]', `Test ${questionType} question`);
                    ...
                    await expect(page.locator(`text=Test ${questionType} question`)).toBeVisible();
                });

                test(`API: ${questionType} question saves correctly`, async ({ testDb, authToken }) => {
                    const question = testDataBuilders.postsortQuestion(questionType, `Test ${questionType}`, {
                        required: true,
                    });
                    ...
                    expect(study.postsort_config.questions[`test_${questionType}`].type).toBe(questionType);
                });

                test(`Participant: ${questionType} question renders correctly`, async ({ page, testDb, authToken }) => {
                    const question = testDataBuilders.postsortQuestion(questionType, `Participant ${questionType} Test`);
                    ...
                    await expect(page.locator(`text=Participant ${questionType} Test`)).toBeVisible();
                });

                test(`Validation: Required ${questionType} question enforced`, async ({ page, testDb, authToken }) => {
                    const question = testDataBuilders.postsortQuestion(questionType, `Required ${questionType}`, {
                        required: true,
                    });
