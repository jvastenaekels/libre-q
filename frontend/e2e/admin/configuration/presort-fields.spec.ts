    ...
    for (const fieldType of FIELD_TYPES) {
        test.describe(`Field Type: ${fieldType}`, () => {
            ...
            test(`Admin: Can add ${fieldType} field`, async ({ page, authToken }) => {
                ...
                await expect(page.locator(`text=Test ${fieldType} Field`)).toBeVisible();
            });
            ...
            test(`API: ${fieldType} field is saved correctly`, async ({ testDb, authToken }) => {
                ...
                await testDb.updateStudy(authToken, studySlug, { presort_config: presortConfig });
                ...
                expect(study.presort_config.fields[`test_${fieldType}`].label).toBe(`Test ${fieldType} Field`);
                expect(study.presort_config.fields[`test_${fieldType}`].required).toBe(true);
            });

            test(`Participant: ${fieldType} field renders correctly`, async ({ page, testDb, authToken }) => {
                // Setup: Add field via API
                const field = testDataBuilders.presortField(fieldType, `Participant ${fieldType} Test`);
                const presortConfig = testDataBuilders.presortConfig({
                    [`test_${fieldType}`]: field,
                });
            ...
            test(`Validation: ${fieldType} field validation works`, async ({ page, testDb, authToken }) => {
            ...
            test(`Edge Case: ${fieldType} field with constraints`, async ({ page, testDb, authToken }) => {
                ...
                if (fieldType === 'number') {
                    ...
                    edgeCaseValue = { valid: 'test@example.com', invalid: 'not-an-email' };
                } else {
                ...
                if (edgeCaseValue.invalid) {
                    ...
                    await page.click('button:has-text("Continue")');
                    await expect(page.locator('text=invalid', { hasText: /invalid|error/i })).toBeVisible();
                }
