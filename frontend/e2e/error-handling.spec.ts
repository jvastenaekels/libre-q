...
test.describe('Error Handling Scenarios', () => {
  ...
  test('should handle API rate limiting (429)', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Too Many Requests|Trop de requêtes/i })).toBeVisible();
    
    // Should see specific message
    await expect(page.getByText(/You are doing that too fast|Vous allez trop vite/i)).toBeVisible();
  ...
  test('should handle network connection errors', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /Connection Lost|Connexion perdue/i })).toBeVisible();
