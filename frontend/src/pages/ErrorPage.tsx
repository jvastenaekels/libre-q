...
const ErrorPage: React.FC<ErrorPageProps> = ({ 
  ...
  const { title, message, icon: Icon, showReset, showHome, showRetry } = useMemo(() => {
  ...
  const containerProps = isFullPage ? { className: "min-h-screen bg-gray-50 flex items-center justify-center p-4" } : {};

  return (
    <Container {...containerProps}>
      <div className={`${isFullPage ? 'max-w-md w-full bg-white rounded-2xl shadow-xl p-8' : 'w-full p-6 text-center'} text-center space-y-6`}>
        <div className={`w-20 h-20 ${error instanceof ApiError && error.status === 404 ? 'bg-orange-100 text-orange-600' : 'bg-red-100 text-red-600'} rounded-full flex items-center justify-center mx-auto transition-colors`}>
          <Icon size={40} />
        </div>
